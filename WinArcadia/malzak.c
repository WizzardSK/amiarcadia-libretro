...
{   int where,
    ...
    zprintf(TEXTPEN_VIEW, "Screen:\n");
    for (where = 0x1800; where < 0x1E00; where += 16)
    {   dumprow(where); // ideally, we would show the text in the correct colours
        if (where % 64 == 48)
        {   zprintf(TEXTPEN_VIEW, "\n");
    }   }

    zprintf(TEXTPEN_VIEW, "Playfield:\n");
    for (y = 0; y < 16; y++)
    {   zprintf
        (   TEXTPEN_VIEW,
...
{   int   i, address, mirror;
    ...
    {   for (i = 0; i <= 0xBFF; i++)
        ...
        cd_configs();
        if ((TheLocalHandle = fopen("Malzak2.nvr", "rb")))
        {   DISCARD fread(&memory[0x1700], 256, 1, TheLocalHandle);
            DISCARD fread(&memory[0x14CC],   1, 1, TheLocalHandle); // potentiometer
            DISCARD fclose(TheLocalHandle);
            // TheLocalHandle = NULL;
#ifdef VERBOSE
            zprintf(TEXTPEN_VERBOSE, "Loaded NVRAM.\n");
#endif
        } else
        {   for (i = 0; i <= 255; i++)
...
{   TRANSIENT UBYTE t;
    ...
    {   old_coina        = coinop_coina;
        ...
        {   holding = FALSE;
            ...
            {   if (shift())
                {   switch (memory[0x1400 + PVI_P1PADDLE])
                    {
                    case 0xF0:  // state "1", change to "4"
                        memory[0x1400 + PVI_P1PADDLE] = 0x00;
                    acase 0x90: // state "2", change to "1"
                        memory[0x1400 + PVI_P1PADDLE] = 0xF0;
                    acase 0x70: // state "3", change to "2"
                        memory[0x1400 + PVI_P1PADDLE] = 0x90;
                    adefault:   // state "4" (eg. $00), change to "3"
        ...
        {   coinop_record();
            OutputBuffer[2] = memory[0x14CC]; // test switch
...
{   FILE* TheLocalHandle; // LocalHandle is a variable of winbase.h
    ...
    cd_configs();
    if ((TheLocalHandle = fopen("Malzak2.nvr", "wb")))
    {   DISCARD fwrite(&memory[0x1700], (size_t) 256, 1, TheLocalHandle);
        DISCARD fwrite(&memory[0x14CC],            1, 1, TheLocalHandle); // test switch
        DISCARD fclose(TheLocalHandle);
        // TheLocalHandle = NULL;
#ifdef VERBOSE
        zprintf(TEXTPEN_VERBOSE, "Saved NVRAM.\n");
#endif
    }
    cd_progdir();
}
    ...
    {   for (y = 0; y < 16; y++)
        {   for (x = 0; x < 16; x++)
            {   ssx = ((479 + (x * 16) - malzak_x) % 256) - 15 + offsetsprites;
                ...
                {   for (xx = 0; xx < 16; xx++)
                    {   if
                        (   (xx == 0 || xx == 15 || yy == 0 || yy == 15)
                         && ssx + xx >= 0
                         && ssx + xx <  COINOP_BOXWIDTH
                         && ssy + yy >= 0
...
{   ULONG temp;
    ...
    acase 0x90: temp = 1;
    acase 0x70: temp = 2;
    adefault:   temp = 3; // eg. $00
    }
    ra_set(SUBWINDOW_DIPS, IDC_SWITCH_1, IDC_SWITCH_4, temp);
}
